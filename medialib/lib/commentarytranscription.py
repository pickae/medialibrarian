...
def export_commentary(directory: str, read_track_info, is_bonus_folder,
    ...
    a second time. ``orphans``, when given, is a list the walk fills with a
    transcript that names a track the film no longer numbers a commentary for;
    the -c run is the one that hands it one, so it can leave the list in a
    file. Everything else is this run's own: the flat queue spans every movie
    and every language wanted, so the workers stay busy to the last record.
    """
    try:
        os.chdir(directory)
    except OSError:
    ...
    for file in files:
        ...
        # A transcript that names a track number the film no longer numbers a
        # commentary for is one the film has outlived: reported, not transcribed
        # and not silently kept. The walk only asks when the run wants the list.
        ...
        for i in range(1, len(comments) + 1):
            ...
            # determine name of file to export; a nameless track is the rare
            # one, and the shell's "null" would be spelled into the name as-is
