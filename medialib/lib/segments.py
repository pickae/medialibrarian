...
Everything except ``detectWindow``, which is an ffmpeg decode rather than a
calculation.
...
with no shared state, and the two passes that turn a duration into a chunk
count and a list of silence-nudged cut points.

Both passes read their inputs with the same coercion - a duration of "" or
"later" is zero, not an error - so a missing probe result plans a file of
length zero rather than stopping the run. That is a deliberate property, and
the one coercion rule in ``formatting.awk_number`` keeps it, including where
that rule departs from gawk on values too large for a double.
"""
...
def seg_plan(duration: object, jobs: object) -> str | None:
    ...
    truncated, while the segment length divides by the value itself. A fractional
    core count is not a thing a machine reports, but the two readings have to
    stay two: rounding once would change the segment length the moment anything
    passed a fraction through.
...
def _first_field(line: str) -> str:
    """The first whitespace-separated field, or "" for a blank line.

    Reading the whole line would give the same number - the numeric prefix
    cannot cross a blank, which is also what separates the fields.
    """
    fields = line.split()
...
def select_boundaries(
    ...
    # A numeric sort, whose tie-break between equal numbers is the whole
    # line's bytes. The tie-break cannot change the answer (equal numbers
    # are interchangeable once read).
